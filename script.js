let cartItems = JSON.parse(sessionStorage.getItem('cart')) || []; // Cargar el carrito desde sessionStorage o inicializar vacío
...
function addToCart(productName, price) {
    ...
    sessionStorage.setItem('cart', JSON.stringify(cartItems)); // Guardar carrito en sessionStorage
    renderCart();
}
...
function renderCart() {
    ...
    if (cartItems.length === 0) {
        ...
        sessionStorage.removeItem('cart'); // Limpiar carrito de sessionStorage si está vacío
        return;
    }
...
function proceedToCheckout() {
    alert('¡Gracias por tu compra! Los productos se eliminarán del carrito en 5 segundos.');

    // Esperar 5 segundos antes de vaciar el carrito
    setTimeout(() => {
        cartItems = []; // Vaciar el carrito
        sessionStorage.removeItem('cart'); // Borrar carrito de sessionStorage
        renderCart(); // Actualizar la interfaz del carrito
    }, 5000); // 5 segundos en milisegundos

    setTimeout(() => {
        window.location.href = 'formulario_pedido.html'; // Redirigir al formulario
    }, 1000); // Redirigir inmediatamente
}
